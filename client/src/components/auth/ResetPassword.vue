...
// 폼 상태 (초기값)
const verifyData = ref({ name: '', email: '', phone: '' });
const resetData = ref({ newPassword: '', confirmPassword: '' });
...
// 상태 관리
const step = ref(1);
const findMethod = ref<'email' | 'phone'>('email');

const isLoading = ref(false); // 로딩 상태
const errorMessage = ref(''); // 에러 메시지 상태
const targetUserId = ref(''); // 서버에서 받아온 유저 식별 ID
...
const updateEmail = () => {
  if (emailUser.value && emailDomain.value) {
    verifyData.value.email = `${emailUser.value}@${emailDomain.value}`;
  } else {
    verifyData.value.email = '';
  }
};
...
const handlePhoneInput = (e: Event) => {
  ...
  // 길이에 따라 하이픈 삽입
  if (value.length < 4) {
    verifyData.value.phone = value;
  } else if (value.length < 8) {
    verifyData.value.phone = `${value.slice(0, 3)}-${value.slice(3)}`;
  } else {
    // 최대 11자리 제한
    value = value.slice(0, 11);
    verifyData.value.phone = `${value.slice(0, 3)}-${value.slice(3, 7)}-${value.slice(7)}`;
  }
};
...
// 탭 변경
const setMethod = (method: 'email' | 'phone') => {
  findMethod.value = method;
  errorMessage.value = '';

  verifyData.value.email = '';
  verifyData.value.phone = '';
  emailUser.value = '';
};
...
const handleVerifyUser = async () => {
  ...
  try {
    const payload = {
      ...
      [findMethod.value]: findMethod.value === 'email' ? verifyData.value.email : verifyData.value.phone.replace(/-/g, '')
    };
    const response = await axios.post(`${BASE_URL}/verify-for-password`, payload);
    targetUserId.value = response.data.userId;
    step.value = 2; // 다음 단계로 이동
  } catch (error: any) {
    errorMessage.value = error.response?.data.message || '정보가 일치하지 않습니다.';
  } finally {
...
const handleResetPassword = async () => {
  ...
  errorMessage.value = '';
  try {
    await axios.patch(`${BASE_URL}/reset-password-unauthenticated`, {
    ...
    errorMessage.value = '변경 중 오류가 발생했습니다.';
  } finally {
    isLoading.value = false;
  }
};
...
<template>
  <div class="find-wrapper">
    <div class="find-container">
      ...
      <div v-if="step === 1">
        <div class="tab-group">
          <button type="button" :class="{ active: findMethod === 'email' }" @click="setMethod('email')">이메일 인증</button>
          <button type="button" :class="{ active: findMethod === 'phone' }" @click="setMethod('phone')">휴대폰 인증</button>
        </div>
        
        <form @submit.prevent="handleVerifyUser" class="find-form">
          <div class="input-group-stack">
            <div class="field-wrap">
              <label>이름</label>
              <input type="text" v-model="verifyData.name" placeholder="이름을 입력해주세요" required />
            </div>

            <div v-if="findMethod === 'email'" class="field-wrap">
              <label>이메일</label>
              <div class="email-input-wrap">
                <input type="text" v-model="emailUser" @input="updateEmail" placeholder="아이디" required />
                <span class="at">@</span>
            ...
            <div v-else class="field-wrap">
              <label>휴대폰 번호</label>
              <input type="tel" :value="verifyData.phone" @input="handlePhoneInput" @keydown="onlyNumberKey" placeholder="숫자만 입력해주세요" required maxlength="13" />
            </div>
          </div>
          ...
          <button type="submit" class="submit-btn" :disabled="isLoading">확인</button>
          <button type="button" class="outline-btn" @click="router.push('/login')">로그인</button>
        </form>
      </div>

      <div v-else-if="step === 2">
        <div class="info-msg">새로운 비밀번호를 입력해주세요.</div>
        <form @submit.prevent="handleResetPassword" class="find-form">
          <div class="input-group-stack">
            <input type="password" v-model="resetData.newPassword" placeholder="새 비밀번호" required />
            <input type="password" v-model="resetData.confirmPassword" placeholder="새 비밀번호 확인" required />
          </div>
          <p v-if="errorMessage" class="error-message">{{ errorMessage }}</p>
          <button type="submit" class="submit-btn" :disabled="isLoading">비밀번호 변경</button>
        </form>
      </div>
    </div>
  </div>
